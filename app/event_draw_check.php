...
<main class="w-auto m-auto">
    ...
    if($event_id != "" && $tickets != ""){
        $validate_error_msg = '';
        if (!isIntValid($tickets) || $tickets == '0') {
            $validate_error_msg = "Invalid number of tickets";
        } elseif (!isIntValid($event_id) || $event_id == "0") {
        ...
        if ($validate_error_msg){
            ...
            <?php
            header( "refresh:3;url=event_details.php?id=".$event_id );
            exit();
        }
        $myEvent = new MyEvent();
        ...
        if(!empty($event_details)) {
            ...
            if (!empty($participants)) {
                $winners = drawWinners($participants, $tickets);
                $saved = saveWinners($winners, $event_id);
                if ($saved) {
                    setDrawDate($event_id);
                ?>
                ...
                <div class="card border-danger mb-3">
                    ...
                    <div class="card-body">
                        <p class="card-text text-center fs-4 text-nowrap">There was an error drawing tickets. Please try again.</p>
                    </div>
                </div>
                <?php
                }
